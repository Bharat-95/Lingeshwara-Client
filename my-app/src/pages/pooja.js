 ...
 useEffect ( () => {
  ...
  const fetchData =  async () => {
    ...
    try {
      ...
      const response = await fetch('http://20.197.16.153:4000/pooja')
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(`http://localhost:4000/pooja/${id}`, {
        method: 'DELETE',
      });
...
if(data)
  ...
  {data.map((Pooja) => (
    <div key={Pooja._id} className="m-4">
      <div>
        <Image src={`data:image/jpeg;base64,${Pooja.image}`} alt="Image" className="w-auto h-96 pb-4" />
        <div className="text-center text-xl pb-2">{Pooja.Title}</div>
        <div className="text-center pb-2">{Pooja.Price}</div>
        <button onClick={() => handleDelete(Pooja._id)} className="ml-[85px]">
          Delete
        </button>
      </div>
    </div>
