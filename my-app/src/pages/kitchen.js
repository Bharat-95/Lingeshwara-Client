 ...
 useEffect ( () => {
  ...
  const fetchData =  async () => {
    ...
    try {
      ...
      const response = await fetch('http://13.233.184.29:4000/kitchen')
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(`http://localhost:4000/kitchen/${id}`, {
        method: 'DELETE',
      });
...
if(data)
  ...
  {data.map((Kitchen) => (
    <div key={Kitchen._id} className="m-4">
      <div>
        <img src={`data:image/jpeg;base64,${Kitchen.image}`} alt="Image" className="w-auto h-96 pb-4" />
