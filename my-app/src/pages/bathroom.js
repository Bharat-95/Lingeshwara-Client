 ...
 useEffect ( () => {
  ...
  const fetchData =  async () => {
    ...
    try {
      ...
      const response = await fetch('http://13.233.184.29:4000/bathroom')
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('http://localhost:4000/bathroom', {
        method: "POST",
        body: formDataToSend,
      });
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(`http://localhost:4000/bathroom/${id}`, {
        method: 'DELETE',
      });
...
if(data)
  ...
  {data.map((Bathroom) => (
    <div key={Bathroom._id} className="m-4">
      <div>
        <img src={`data:image/jpeg;base64,${Bathroom.image}`} alt="Image" className="w-auto h-96 pb-4" />
